 ...
 * lives in `smd-react`'s `registry.ts`, which normalizes `''` to `null` on
 ...
 * rendering — `smd-react` (or any other renderer) turns this hast tree into
 * its own component tree, resolving `<smd-directive>` elements through a
 * registry.
 */
