 ...
 *
 * `note.smd` and `manifest.json` are denied unconditionally, regardless of
 ...
 * could edit `note.smd` would make the document self-modifying, which §8
...
export function isWriteAllowed(
  ...
  if (p === 'note.smd' || p === 'manifest.json') return false;
