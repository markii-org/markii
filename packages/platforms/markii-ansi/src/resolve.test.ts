...
/**
 * `@markii/runtime`'s concrete `createValueStore`/`createVaultStore` are
 * used here purely as convenient, already-correct TEST DOUBLES: their
 * return values satisfy `./value-types.js`'s structural `AnsiValueStore`/
 * `AnsiVaultStore` (see `value-types.drift.test.ts`), and `@markii/runtime`
 * is a devDependency of this package for exactly this reason. Nothing in
 * `src/resolve.ts` itself imports `@markii/runtime`.
 */
