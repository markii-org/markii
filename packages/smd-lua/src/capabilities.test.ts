...
describe('buildCapabilities — bundle delegates to a real smd-bundle ScriptView', () => {
  ...
  it("tier 'manual': bundle.write to manifest.json is blocked by smd-bundle's own policy (ScriptCapabilityError), not by this package reimplementing it", async () => {
    const { view } = fixtureBundle();
  ...
  it("tier 'manual': bundle.write to note.smd is blocked", async () => {
    const { view } = fixtureBundle();
    const r = await run('bundle.write("note.smd", "# hacked"); return true', {
      tier: 'manual',
      bundle: view,
    });
    expect(r.ok).toBe(false);
