 ...
 * ∩ user grant — same pattern as `smd-bundle`'s `createScriptView`, DEFECT
...
export interface CapabilityConfig {
  ...
  netGrants?: NetGrants;
  cache?: CacheProvider;
  /** Bundle-scoped filesystem view (spec §11) — already capability-restricted by `smd-bundle`'s `createScriptView`; this module delegates to it, never re-implements the path-jail or write policy. */
  bundle?: ScriptView;
...
`);
  ...
  // --- bundle -------------------------------------------------------------
  // Delegates entirely to the injected `ScriptView` (`smd-bundle`), which
  ...
  if (config.bundle) {
    ...
    // `ScriptView` (smd-bundle) throws its own `ScriptCapabilityError` /
