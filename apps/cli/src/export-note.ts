/**
 * `markii export`'s three formats.
 *
 * `html` goes through `@markii/host`'s `buildNoteHtmlExport` so the file is
 * byte-identical to what the GUI hosts export for the same note and
 * values. `ansi` is `render-note.ts`'s output, written to a file (color
 * defaults to `none` for a file, since nobody's terminal is reading it
 * directly; an explicit `--color` is still honoured). `md-plain` is a
 * SOURCE-LEVEL rewrite: no markdown serializer, no new dependency — every
 * directive's own fence/bracket syntax is deleted from the original text
 * using the offsets `@markii/core`'s `parse` already puts on every node,
 * leaving the author's own inner markdown untouched.
 */
...
import { buildNoteHtmlExport, type NoteHtmlExportOptions } from '@markii/host';
import type { StoredValue } from '@markii/runtime';

/** `buildNoteHtmlExport`'s exact inputs, so `markii export --format html` reuses the shared host export path verbatim. */
export function exportHtml(
  text: string,
  fileName: string,
  values?: Record<string, StoredValue>,
): string {
  const options: NoteHtmlExportOptions = { text, fileName };
  if (values !== undefined) {
    return buildNoteHtmlExport({ ...options, values });
  }
  return buildNoteHtmlExport(options);
}
 ...
 * faithful render (see `README.md`'s ceiling note).
 *
