...
import type { PackBuildOptions } from './pack-build.js';
...
function packFor(folder: string): DiscoveredPack {
  return {
    folder,
    manifest: {
      name: 'fakepack',
...
/** A fake `build` that counts invocations and returns a fixed, well-formed IIFE — lets cache-hit/miss and wiring be tested fast, without paying for a real esbuild-wasm invocation (that path is covered separately, against the real thing, in `fixture-integration.test.ts`). */
