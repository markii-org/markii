 ...
 * discovery and a REAL esbuild-wasm invocation (no fakes — that is
 ...
 * directly via `tsx/cjs` — the SAME dev-mode mechanism
 * `../run/run-host.ts`'s `TSX_DEV_EXEC_ARGV` already uses to run a
 * `worker_threads` worker straight from `.ts` source), never inside
 * Vitest's own module realm. This is not a style choice: esbuild-wasm's
 * Node API runs a startup invariant check
 * (`new TextEncoder().encode('') instanceof Uint8Array`) that fails when
 * evaluated inside Vitest's per-file `vm` module-transform context —
 * confirmed empirically against this exact environment (plain
 * `environment: 'node'`, no jsdom involved, and true regardless of
 * whether esbuild-wasm is loaded via a static import or a genuine runtime
 * `require()`) — because typed-array intrinsics differ across `vm`
 * realms even when the requiring code path is otherwise ordinary. The
 * PRODUCTION path never hits this: the VS Code extension host is a plain,
 * unsandboxed Node process, not a Vitest worker.
 *
...
const DISCOVER_TS = path.join(import.meta.dirname, 'discover.ts');

/** Runs the real `discoverPacks` + `buildPackRegistrationScript` in a genuinely separate `node` process (see this file's top doc comment for why). Writes a small throwaway driver script into `workDir` and shells out to it. */
function buildInChildProcess(
  ...
  const driverSource = `
    const { discoverPacks, createNodeFileReader } = require(${JSON.stringify(DISCOVER_TS)});
    const { buildPackRegistrationScript } = require(${JSON.stringify(PACK_BUILD_TS)});
    (async () => {
      const { packs } = await discoverPacks([${JSON.stringify(packDir)}], createNodeFileReader());
      if (packs.length !== 1) {
        process.stdout.write(JSON.stringify({ kind: 'failed', reason: 'fixture did not discover exactly one pack' }));
        return;
      }
      const outcome = await buildPackRegistrationScript(packs[0], ${JSON.stringify(cacheDir)});
